...
async function iniciarLlamadaAxiomAI(formData: FormData) {
  ...
  if (!apiKey) throw new Error("Falta TELNYX_API_KEY.");
  if (!texmlAppId) throw new Error("Falta TELNYX_TEXML_APP_ID.");
  if (!assistantId) throw new Error("Falta TELNYX_AI_ASSISTANT_ID.");
  if (!fromNumber) throw new Error("Falta TELNYX_FROM_NUMBER.");
  if (!crmCallPin) throw new Error("Falta AXIOMOS_CRM_CALL_PIN.");
  ...
  const numeroOrigen = telefonoE164(fromNumber);
  ...
  console.log("=== AXIOMOS OUTBOUND: ENVIANDO A TELNYX ===", {
    prospectoId,
    telefono,
    prospectoNombre,
    prospectoEmpresa,
    assistantIdConfigurado: Boolean(assistantId),
    texmlAppIdConfigurado: Boolean(texmlAppId),
  });
    ...
    )}`,
    {
      ...
      body: JSON.stringify({
        ...
        AIAssistantDynamicVariables: {
          ...
          prospect_company: prospectoEmpresa || "No disponible",
          crm_notes:
            prospectoNotas || "No hay notas internas disponibles",
  ...
  console.log("=== AXIOMOS OUTBOUND: RESPUESTA TELNYX ===", {
  ...
  console.log("=== AXIOMOS OUTBOUND: LLAMADA INICIADA ===", {
    prospectoId,
    telefono,
    status: response.status,
  });
